...
const CreateStore = () => {
  ...
  return (
    <div className="w-full h-[100vh] flex flex-col">
      <div className="bg-accent flex items-center justify-between w-full text-white text-xl md:text-2xl">
        <h1 className="p-2">Create store</h1>
        <button onClick={()=> router.replace("/")} className="bg-white mr-4 h-[35px] w-[35px] rounded-full flex items-center justify-center">
      ...
      <div className="w-full h-full flex md:flex-row flex-col overflow-x-hidden overflow-y-auto custom_scroll max-w-[90rem]">
        <div className="w-[100%] md:w-[70%] mx-auto h-full flex flex-col p-4 items-center justify-center">
          <div className="w-[90%] max-w-[30rem] flex flex-col items-center">
            <img src="/svgs/become_seller.svg" className="w-full" alt="become seller" />
          </div>
            ...
            <ul className="p-2 w-[100%] md:w-[90%]">
              {
                SellerFeatures.map(feature=> (
                  <li className="list-decimal text-sm" key={feature}>{feature}</li>
                ))
              }
            </ul>
          </div>
        </div>
        <form className="flex flex-col p-8 mx-auto max-w-[20rem] md:w-full" onSubmit={handleSubmit}>
          <label htmlFor="store_name">What would you like to call your store?</label>
          <input
            id="store_name"
            type="text"
            ref={inputRef}
          ...
          <div className="mt-2">
            {
              StoreNameRules.rules.map(rule=>(
                <h2 key={rule} className="text-sm">{rule}</h2>
              ))
            }
          </div>
        </form>
